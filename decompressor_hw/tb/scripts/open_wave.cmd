...
if not exist "%SIM_DIR%\xsim.dir\decompressor_sim" (
    echo Simulation snapshot missing. Running simulation first...
    call "%SCRIPTS_DIR%run_sim.cmd"
    if errorlevel 1 exit /b 1
)

if not exist "%TB_DIR%\wave\decompressor_tb.vcd" if not exist "%TB_DIR%\wave\decompressor_sim.wdb" (
...
echo   VCD: %TB_DIR%\wave\decompressor_tb.vcd
echo   WDB: %TB_DIR%\wave\decompressor_sim.wdb
echo.

set "SIM_DIR=%SIM_DIR%"
set "VIVADO_ROOT=%VIVADO_ROOT%"
start "decompressor waves" "%SCRIPTS_DIR%launch_wave_gui.cmd"
