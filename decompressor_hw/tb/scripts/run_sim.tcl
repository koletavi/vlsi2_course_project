# run_sim.tcl — Run decompressor testbench (use from Vivado Tcl console or batch mode)
#
# From Vivado GUI Tcl Console:
#   cd C:/Users/kolet/projects/vlsi2/decompressor_hw/tb
#   source run_sim.tcl
#
# From command line:
...
set sv_files [list \
    ...
    [file join $src_dir decompressor_ref_pkg.sv] \
    [file join $src_dir decompressor_tb.sv] \
]
...
run_cmd "Elaborating (xelab)" [list $xelab -debug typical decompressor_tb -s decompressor_sim -log $xelab_log]
...
run_cmd "Simulating (xsim)" [list $xsim decompressor_sim -tclbatch $sim_tcl -log $xsim_log]

# Copy WDB snapshot(s) from sim_out into wave/ for offline viewing
foreach wdb [glob -nocomplain -directory $sim_dir *.wdb] {
    set dest [file join $wave_dir decompressor_sim.wdb]
