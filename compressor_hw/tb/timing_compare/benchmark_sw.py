#!/usr/bin/env python3
"""Benchmark software compression time using compressor_sw/posit_compress.py."""
...
from generate_sw_results import compress_packet_sw, parse_original_packet  # noqa: E402

WARMUP_ITER = 100
TIMED_ITER = 10_000


def bench_one(words: list[int]) -> dict[str, float]:
    for _ in range(WARMUP_ITER):
        compress_packet_sw(words)
...
def format_report(results: list[tuple[str, dict[str, float]]]) -> str:
    ...
    lines = [
        ...
        sep,
        "Source:      compressor_sw/posit_compress.py (DIFFNB -> BIT -> RZE)",
        f"Generated:   {now}",
