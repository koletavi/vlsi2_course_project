...
if (-not (Test-Path (Join-Path $SimDir "xsim.dir\compressor_sim"))) {
    Write-Host "Simulation snapshot not found. Running simulation first..."
    & (Join-Path $ScriptsDir "run_sim.ps1")
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}

$WaveDb  = Join-Path $TbDir "wave\compressor_sim.wdb"
$VcdFile = Join-Path $TbDir "wave\compressor_tb.vcd"
